...
def _get_tile(tile_str: str, position: tuple[int, int]) -> Tile:
    match tile_str:
        case '|': return PipeTile([TileSide.NORTH, TileSide.SOUTH], position)
        case '-': return PipeTile([TileSide.WEST, TileSide.EAST], position)
        case 'L': return PipeTile([TileSide.NORTH, TileSide.EAST], position)
        case 'J': return PipeTile([TileSide.NORTH, TileSide.WEST], position)
        case '7': return PipeTile([TileSide.WEST, TileSide.SOUTH], position)
        case 'F': return PipeTile([TileSide.EAST, TileSide.SOUTH], position)
...
def _update_connectors(connectors: list[TileSide], graph: TileGraph, offset: tuple[int, int], side: TileSide) -> None:
    ...
    assert isinstance(neighbouring_tile, PipeTile)
    if neighbouring_tile.has(side): connectors.append(side.get_opposite())

def _populate_starting_tile(graph: TileGraph) -> None:
    s_i = graph.starting_position[0]
    s_j = graph.starting_position[1]
    connectors = []
    _update_connectors(connectors, graph, (-1, 0), TileSide.SOUTH)
    _update_connectors(connectors, graph, (1, 0), TileSide.NORTH)
    _update_connectors(connectors, graph, (0, -1), TileSide.EAST)
    _update_connectors(connectors, graph, (0, 1), TileSide.WEST)
